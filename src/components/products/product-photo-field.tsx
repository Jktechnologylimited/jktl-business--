...
/** Square product photo shown on both the internal Products list and the
 * public website's catalog. Same client-crop-then-upload-on-save pattern
 * as `LogoUploadField`; falls back to a generic package icon rather than
 * initials since a product has no name-based avatar to fall back to. */
export function ProductPhotoField({ value, onChange }: { value: string | null; onChange: (dataUrl: string | null) => void }) {
  ...
  async function handleFile(file: File | undefined) {
    ...
    setBusy(true);
    try {
      const dataUrl = await fileToSquareDataUrl(file, 480);
      onChange(dataUrl);
    } catch (err) {
    ...
    <div className="flex items-center gap-4">
      <div className="relative">
        {value ? (
          ...
          <img src={value} alt="Product" className="size-16 rounded-2xl border border-border-strong object-cover" />
        ) : (
          <span className="flex size-16 items-center justify-center rounded-2xl bg-surface-strong text-ink-faint">
      ...
      <div className="flex flex-col gap-1">
        ...
        {value ? (
          ...
          <span className="text-xs text-ink-muted">Optional — shown on your website</span>
        )}
      </div>
    </div>
  );
